{{-- Just the card, no grid wrapper --}}

    <div
        class="bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 overflow-hidden hover:shadow-md transition-all group flex flex-col rounded-sm">
        ...
        <div class="p-3 flex flex-col flex-1 h-full">
            ...
            <div class="mt-2 pt-1.5 border-t border-gray-100 dark:border-zinc-800">
                <button
                @click="addToCart({ id: {{ $product->id }}, name: '{{ addslashes($product->name) }}', price: {{ $product->selling_price }}, image: '{{ $product->image }}' })"
                ...
                <div x-show="cartItems.find(i => i.id === {{ $product->id }})" x-cloak
                    class="flex items-center gap-2">
                    <button @click="decreaseQty(cartItems.findIndex(i => i.id === {{ $product->id }}))"
                        class="flex-1 py-2 bg-red-100 dark:bg-red-950/90 text-red-600 dark:text-red-100 rounded-md text-sm font-bold">-</button>
                    <span x-text="cartItems.find(i => i.id === {{ $product->id }})?.qty || 0"
                        class="text-base font-bold min-w-[24px] text-center"></span>
                    <button
                        @click="addToCart({ id: {{ $product->id }}, name: '{{ addslashes($product->name) }}', price: {{ $product->selling_price }}, image: '{{ $product->image }}' })"
                        class="flex-1 py-2 bg-green-200 dark:bg-green-800/50 text-green-600 dark:text-green-100 rounded-md text-sm font-bold">+</button>
                </div>
            </div>
        </div>
    </div>
