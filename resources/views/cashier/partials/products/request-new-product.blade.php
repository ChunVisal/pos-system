<div x-show="requestNewProduct" x-cloak class="fixed inset-0 z-50 flex justify-end" style="display: none;">
    ...
    <div @click.stop x-show="requestNewProduct" x-transition:enter="transition transform ease-out duration-300"
        ...
        x-transition:leave="transition transform ease-in duration-250" x-transition:leave-start="translate-x-0"
        ...
        <div class="flex-1 overflow-y-auto px-5 py-4 space-y-4">
            <template x-for="(item, index) in newProductList" :key="index">
                <div
                    ...
                    <div class="space-y-2">
                        <input type="text" required x-model="item.name" placeholder="Product name *"
                            class="w-full text-xs font-semibold border border-gray-250 dark:border-zinc-800/80 rounded-md px-3 py-2 bg-white dark:bg-zinc-950 text-gray-900 dark:text-zinc-100 focus:outline-none focus:border-[#0F6E8C] dark:focus:border-[#1389af] transition-colors">

                        <div class="flex gap-2">
                            <input type="number" x-model="item.quantity" placeholder="Qty" min="1"
                                class="w-20 text-xs font-semibold text-center border border-gray-250 dark:border-zinc-800/80 rounded-md px-2 py-2 bg-white dark:bg-zinc-950 text-gray-900 dark:text-zinc-100 focus:outline-none focus:border-[#0F6E8C] dark:focus:border-[#1389af] transition-colors [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none">
                            <input type="text" x-model="item.note" placeholder="Note (optional)"
                                class="flex-1 text-xs font-semibold border border-gray-250 dark:border-zinc-800/80 rounded-md px-3 py-2 bg-white dark:bg-zinc-950 text-gray-900 dark:text-zinc-100 focus:outline-none focus:border-[#0F6E8C] dark:focus:border-[#1389af] transition-colors">
                        </div>
                    </div>
                </div>
            </template>
        ...
        <div
            ...
            <button @click="requestNewProduct = false"
                class="flex-1 py-2 text-[11px] font-bold uppercase tracking-wider text-gray-500 dark:text-zinc-400 hover:text-gray-900 dark:hover:text-zinc-100 border border-gray-250 dark:border-zinc-800/80 rounded-md transition-colors">
                Cancel
            </button>
