<div x-data="{ loading: true }" x-init="setTimeout(() => loading = false, 500)" x-cloak>
    ...
    <div x-show="loading" x-transition:leave="transition-opacity duration-300" x-transition:leave-start="opacity-100"
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <div>
                <div class="h-8 w-40 bg-gray-200 dark:bg-zinc-700 rounded animate-pulse"></div>
        ...
        <!-- Tabs + Filters -->
        <div class="mt-4 bg-white dark:bg-zinc-900 rounded-md shadow-sm border border-gray-200 dark:border-zinc-800/60">
            <div class="border-b border-gray-200 dark:border-zinc-800 p-4">
                <div class="flex items-center gap-6">
                    @for ($i = 0; $i < 3; $i++)
                        <div class="h-8 w-28 bg-gray-200 dark:bg-zinc-700 rounded animate-pulse"></div>
                    @endfor
                ...
                <div class="space-y-4">
                    @for ($i = 0; $i < 3; $i++)
                        <div>
                            ...
                            @for ($j = 0; $j < 3; $j++)
                                <div
                                    class="flex items-start gap-3 py-3 border-b border-gray-100 dark:border-zinc-800/50">
                                    <div
                                        class="w-8 h-8 bg-gray-200 dark:bg-zinc-700 rounded-full animate-pulse shrink-0">
                                    </div>
                                    <div class="flex-1">
                                        <div class="flex items-center gap-2">
                                            <div class="h-4 w-24 bg-gray-200 dark:bg-zinc-700 rounded animate-pulse">
                                            </div>
                                            <div class="h-3 w-32 bg-gray-200 dark:bg-zinc-700 rounded animate-pulse">
                                            </div>
                                            <div class="h-3 w-16 bg-gray-200 dark:bg-zinc-700 rounded animate-pulse">
                                            </div>
                                        </div>
                                        <div class="flex items-center gap-3 mt-1">
                                            <div class="h-3 w-20 bg-gray-200 dark:bg-zinc-700 rounded animate-pulse">
                                            </div>
                                            <div
                                                class="h-5 w-16 bg-gray-200 dark:bg-zinc-700 rounded-full animate-pulse">
                                            </div>
                                        </div>
                                    </div>
                                    <div class="h-6 w-6 bg-gray-200 dark:bg-zinc-700 rounded animate-pulse shrink-0">
                                    </div>
                                </div>
                            @endfor
                        </div>
